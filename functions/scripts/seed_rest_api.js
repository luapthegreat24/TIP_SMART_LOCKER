...
async function seedLockers() {
  console.log("[SEED] Creating bootstrap lockers via REST API\n");
  console.log("NOTE: Using PATCH with full path and merge=true\n");
  ...
  for (const locker of BOOTSTRAP_DATA) {
    try {
      ...
      // Use PATCH with full document path
...
seedLockers()
  .then((success) => {
    if (success) {
      ...
      console.log("\n❌ Bootstrap seeding incomplete - check errors above");
      process.exit(1);
    }
  })
  .catch((err) => {
